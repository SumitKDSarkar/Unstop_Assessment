    ...
    <>
      ...
      <div className="w-full border border-dashed group rounded-box bg-[#F6F8FA] p-[1rem] sm:p-3 md:p-[1.875rem] hover:bg-[#ebedf0] cursor-pointer border border-gray-100 border-solid border-2 rounded-lg">
        ...
        >
          ...
          <h2 className=" lg:text-lg text-sm font-semibold lg:font-medium text-[#1C4980]">
            New Assessment
          </h2>
          <p className=" text-xs font-medium text-[#1C4980] text-center">
        ...
        <div>
          ...
          <div
            className={
              "fixed transition-all z-40 duration-500 w-screen h-screen inset-0 bg-gray-900 " +
              (isOpen ? "visible bg-opacity-60" : "invisible bg-opacity-0")
            }
            onClick={closeModal}
          ></div>
          {/* Below code is the modal of form */}
          <form 
            method="dialog"
            className={
              "        p-0 rounded-t-lg transition-all duration-500 md:rounded-lg bg-white w-full fixed z-50 -bottom-[100vh] md:!bottom-auto md:top-[190vh] left-1/2 -translate-x-1/2 max-w-[37rem] md:-translate-y-1/2 " +
              (isOpen ? "!bottom-0 md:!top-1/2" : "")
            }
          >
            <button
              onClick={closeModal}
              className="btn btn-sm min-h-0 h-10 w-10 btn-circle btn-ghost hover:bg-[#FBEBEA] hover:text-[#D63500] bg-[#FBEBEA] text-[#D63500] scale-105 md:bg-transparent md:text-[#1C4980] md:scale-100 absolute md:right-5 md:top-4 right-3 top-3"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="30"
              ...
              <div className="flex gap-5 flex-col form-control w-full">
                <div>
                  <label className="label pt-0">
                    <span className="label-text text-[#1C4980] text-xs md:text-base font-medium">
                      Name of assessment
                    </span>
                  </label>
                  <input
                    ...
                    className="!outline-none input text-[#1C4980] font-medium text-xs  md:text-sm input-bordered w-full"
                  />
                </div>
                <div>
                  <label className="label pt-0">
                  ...
                  <select
                    defaultValue=""
                    className="!outline-none input text-[#1C4980] font-medium text-xs  md:text-sm w-full select select-bordered"
                  >
                  ...
                  <div className="border rounded-lg border-[#DADCE0]">
                    <div className="flex flex-wrap gap-1 md:gap-2 p-4">
                      {skills.map((skill, index) => (
                        <div
                          className="rounded-full flex items-center bg-[#DDEDFF]"
                          key={index}
                        >
                          <span className="text-[#1C4980] text-xs font-medium pl-2 pr-1 md:pl-3 md:pr-2">
                            {skill}
                          </span>
                          <button
                            onClick={(e) => {
                            ...
                            className="btn btn-xs md:btn-sm md:h-7 md:w-7 md:!min-h-0 btn-circle btn-link hover:bg-[#c2d6ee]"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="18"
                    ...
                    <div className="border-t">
                      <input
                        type="text"
                        placeholder="Type Here"
                        className="!outline-none input text-[#1C4980] font-medium text-xs  md:text-sm w-full"
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="px-4 md:px-[1.88rem] py-4 border-t md:border-0 md:shadow-[0_-4px_50px_0_rgba(0,0,0,0.09)]">
              <button className="btn bg-[#0073E6] text-white hover:bg-[#0066cc] transition-all btn-block min-h-[2.6rem] h-4 capitalize">
                Save
              </button>
            </div>
          </form>
      ...
      <button
        className="btn btn-circle fixed right-3 bottom-3 border-none bg-[#0073E6] inline-flex lg:hidden"
        onClick={() => openModal()}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
